  ...
  return (
    <dialog ref={dialogRef} className="storefront-dialog storefront-checkout-dialog" onCancel={(event) => { event.preventDefault(); if (!busy) closeCheckout(); }}>
      ...
      {receipts.length ? <div className="checkout-success">
        ...
        <div className="payment-required-notice"><strong>Upload bukti pembayaran</strong><p>Setelah bayar, kirim bukti transfer di bawah. Setelah selesai, lanjutkan ke konfirmasi WhatsApp agar kami langsung cek ordermu.</p></div>
        ...
        {/^[0-9]{9,15}$/.test(whatsapp) && paymentSubmitted && <a className="commerce-primary whatsapp-button" target="_blank" rel="noreferrer" href={`https://wa.me/${whatsapp}?text=${encodeURIComponent(`Halo Elsewhere, saya ingin konfirmasi pesanan ${orderCodes}. Bukti pembayaran sudah saya kirim dengan total ${rupiah(total)}. Mohon cek dan konfirmasi balik.`)}`}>Konfirmasi ke WhatsApp</a>}
